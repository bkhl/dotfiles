...
(defhydra bkhl/hydra-other-window
  ...
  ("1" delete-other-window "delete other window" :color blue)
...
Package containing the =transpose-frame= command and some others to rearrange windows in a frame.
...
Highlight the region when the mark is active, and make editing commands apply to the selected region.
