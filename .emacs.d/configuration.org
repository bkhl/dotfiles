  ...
  (let* ((name (symbol-name name))
    ...
    (when (or (not (file-exists-p auto-file))
              (let* ((autoloads-attributes
                     ...
                     (autoloads-age
                      (file-attribute-modification-time autoloads-attributes))
                ...
                (seq-find (lambda (attributes)
                          ...
                          (mapcar #'cdr
                                  (directory-files-and-attributes package-directory
                                                                  nil
                                                                  (rx ".el" eos))))))
...
#+begin_src emacs-lisp
(setq my/fixed-pitch-font-family "Source Code Pro"
      my/variable-pitch-font-family "Noto Serif"
      my/variable-pitch-ui-font "Noto Sans")
...
(use-package bug-reference
  ...
  (bug-reference-auto-setup-functions nil)
  :hook
...
(use-package modus-themes
  ...
  :init
  (defun my/set-modus-themes-ui-variable-pitch ()
    (set-face-attribute 'modus-themes-ui-variable-pitch nil
                        :family my/variable-pitch-ui-font))
  :config
  (add-hook 'modus-themes-after-load-theme-hook
            'my/set-modus-themes-ui-variable-pitch)
...
(use-package magit
  ...
  (transient-append-suffix 'magit-push "-M"
    '(4
      "-l"
      "Set all tests label"
      "-omerge_request.unlabel=test::skip -o merge_request.label=test::all"))
  (transient-append-suffix 'magit-push "-l"
    '(4
      "-L"
      "Set skip tests label"
      "-o merge_request.unlabel=test::all -o merge_request.label=test::skip")))
#+end_src
