...
The custom C style here is my default one which is similar to the Linux style but with a 2 character indentation.
...
This is caused by a [[https://web.archive.org/web/20250617022615/https://lists.gnu.org/archive/html/emacs-orgmode/2023-03/msg00299.html][bug discussed on mailing list here]].
...
*** [[https://github.com/r0man/plz-event-source][plz-event-source]]
:PROPERTIES:
:CUSTOM_ID: plz-event-source
:END:
...
*** [[https://github.com/zkry/yaml.el][yaml]]
:PROPERTIES:
:CUSTOM_ID: yaml
:END:
...
(use-package auto-dark
  ...
  (load-theme 'modus-vivendi-tritanopia nil t)
  (auto-dark-mode))
#+end_src
...
(use-package pulsar
  ...
  :bind
  (("M-p" . pulsar-highlight-pulse))
  :hook
  (next-error . pulsar-highlight-pulse)
  :config
...
(use-package go-mode
  :mode
  ((rx ".go" eos) . go-mode)
  ((rx (or "/" bos) "go.mod" eos) . go-dot-mod-mode)
  :config (defun my/config-go-mode ()
            (setq-local tab-width 4)
            (setq-local prettify-symbols-alist
                        `(("<-" . ,my/prettify-left-arrow)
                          ("..." . ,my/prettify-ellipsis))))
  :hook (go-mode . my/config-go-mode))
#+end_src
...
*** [[https://github.com/s-kostyaev/ellama][ellama]] large language model interaction package
:PROPERTIES:
:CUSTOM_ID: ellama
:END:
...
(use-package ellama-transient
  :after ellama
  :load-path "site-lisp/ellama")

(use-package ellama-blueprint
  :after ellama
  :load-path "site-lisp/ellama")
#+end_src
