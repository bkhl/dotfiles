...
Add-on packages are kept as Git submodules in the ~~/.emacs.d/site-lisp/~ directory.
...
The ~use-package~ macro is used to configure most packages later on in this file.
...
The Diminish package allows hiding some lighters from the mode line with the ~:diminish~ keyword for ~use-package~.
...
#+begin_src emacs-lisp
(defvar bkhl/fixed-pitch-font-family "Iosevka BKHL Default")
(defvar bkhl/fixed-pitch-font-size 1.3)

(defvar bkhl/variable-pitch-font-family "Crimson Pro")
(defvar bkhl/variable-pitch-font-size 1.5)
...
(set-face-attribute 'variable-pitch nil
                    ...
                    :height (/ bkhl/variable-pitch-font-size
                               bkhl/fixed-pitch-font-size))
(set-face-attribute 'fixed-pitch nil
                    :family bkhl/fixed-pitch-font-family
                    :height (/ bkhl/fixed-pitch-font-size
                               bkhl/variable-pitch-font-size))
#+end_src
...
In Perl mode, show the double colon operator as ~∷~ (proportion sign), but center it on top of two space characters, to make it same width as the original character.

The hook will only run on graphics displays, because the composed symbol will not work correctly in a terminal. This isn't fool-proof, but works well enough for me as I mostly edit Perl code in a graphical frame.

#+begin_src emacs-lisp
(add-hook 'perl-mode-hook
          (defun bkhl/add-perl-prettify-symbols ()
