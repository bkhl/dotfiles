...
* Servers for integrations with other programs
** Server for the =emacsclient= command
...
** =edit-server= for editing text from other applications

This is primarily for allowing connections from the [[https://github.com/stsquad/emacs_chrome][Edit with Emacs]] extension in Chromium.
...
(use-package edit-server
  ...
  (edit-server-new-frame-alist
   '((window-system . x)))
  :config
  (when (and (not (file-exists-p "/var/run/.containerenv"))
             (daemonp))
    (edit-server-start t)))
#+end_src
