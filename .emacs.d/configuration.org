...
**** Copy element at point

Org has ~org-copy-special~, but it doesn't have special handling of some cases I'd like, for example copying the content of the source/result block at point. This advices that function and will before falling back to the default behaviour try to do ~org-edit-special~ and copy the contents of anything that's opened for editing.

#+begin_src emacs-lisp
(defun my/org-copy-src-code ()
  "Copy content of block at point."
  (interactive)
...
(advice-add #'org-copy-special
            :before-until #'my/org-copy-edit-special)
#+end_src
