...
Enable symbol fonts in order of preference, according to which are available. ☃
...
#+begin_src emacs-lisp
(defun bkhl/set-symbol-fonts-aux (fonts append)
  (when fonts
    (let ((font (car fonts))
          (fonts (cdr fonts)))
      (if (find-font (font-spec :name font))
          (progn
            (set-fontset-font
             t 'symbol font nil (if append 'append nil))
            (bkhl/set-symbol-fonts-aux fonts t))
        (bkhl/set-symbol-fonts-aux fonts append)))))

(defun bkhl/set-symbol-fonts ()
  (bkhl/set-symbol-fonts-aux '("Noto Color Emoji" "Symbola") nil)
  (remove-hook 'focus-in-hook #'bkhl/set-symbol-fonts))

(add-hook 'focus-in-hook #'bkhl/set-symbol-fonts)
#+end_src
...
(defun circe-command-BACKLOG (&optional lines)
  ...
  (let ((lines (or lines 50)))
    (circe-command-MSG
     "*backlog"
     (format "%s %s" circe-chat-target lines))))
#+end_src
