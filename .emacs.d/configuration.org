...
*** Kill current buffer by default with ~C-x C-k~
  ...
  . ((eval
      . (let ((root (project-root (project-current))))
          (setq-local eglot-server-programs
                      `((c-mode "podman" "run" "--rm" "--interactive"
                                ,(concat "--volume=" root ":" root ":z")
                                ,(concat "--workdir=" root)
                                "docker.io/lspcontainers/clangd-language-server:latest")))
          (eglot-ensure))))))
#+end_example
...
*** Macro to add default arguments to built-ins

#+begin_src emacs-lisp
(defmacro my/eshell-default-args (cmd &rest args)
  `(advice-add #',(intern-soft (concat "eshell/" (symbol-name cmd)))
               :filter-args
               (lambda (r)
                 (append ',args r))
               '((name . "default-args"))))
...
(with-eval-after-load 'eshell
  (defun eshell/e (&rest args)
    (dolist (f (eshell-flatten-list (reverse args)))
      (find-file f)))
...
#+begin_src emacs-lisp
(with-eval-after-load 'eshell-unix
  (my/eshell-default-args du "--human-readable"))
#+end_src
...
#+begin_src emacs-lisp
(with-eval-after-load 'eshell-unix
  (my/eshell-default-args grep "--color=auto")
  (my/eshell-default-args egrep "--color=auto")
  (my/eshell-default-args fgrep "--color=auto")

  (defun eshell/pgrep (&rest args)
    (apply #'eshell/grep "--perl-regexp" args)))
#+end_src

**** ~ls~

#+begin_src emacs-lisp
(with-eval-after-load 'eshell-ls
  (my/eshell-default-args ls
                          "--color=auto"
                          "--classify"
                          "--group-directories-first")

  (defun eshell/ll (&rest args)
    (apply #'eshell/ls "-l" args)))
#+end_src
...
*** Languages
**** Emacs Lisp
***** [[file:site-lisp/package-lint][package-lint]] Emacs package linter
...
*** [[file:site-lisp/ob-jira][ob-jira]] Babel support for Jira JQL queries

#+begin_src emacs-lisp
(use-package ob-jira)
#+end_src

Since there is no JQL mode, alias it to ~sql-mode~.

#+begin_src emacs-lisp
(defalias #'jql-mode #'sql-mode)
#+end_src
