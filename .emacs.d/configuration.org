...
(defun my/autoload-package (pkg)
  ...
  (let* ((pkg (symbol-name pkg))
         (pkgdir (expand-file-name (concat user-emacs-directory "site-lisp/" pkg)))
...
These are add-on packages that are dependencies of other packages further down, as declared by the ~:requires~ keywords of those other packages.
...
(use-package magit
  ...
  :after (compat dash transient with-editor)
  :bind
  ("C-x g" . magit-status)
