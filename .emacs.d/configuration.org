...
Note that typically, ~variable-pitch~ is set relative to ~default~, and ~fixed-pitch~ relative to ~variable-pitch~. Consequently, to make the fixed pitch font have the same size in ~variable-pitch~ buffers, the height of ~fixed-pitch~ need to be set to a value that inverses the relative height of the ~variable-pitch~ face.
...
(defun bkhl/set-fontset-fonts ()
  ...
  (set-fontset-font t #x0e3f "Iosevka BKHL Default")
...
**** Elpy for Python IDE functionality
