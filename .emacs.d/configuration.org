...
Load [[https://github.com/myrjola/diminish.el][Diminish]], which allows hiding some mode line displays from the mode line.

For most modes this is configured later on via the ~:diminish~ argument to  ~use-package~.
