...
To use =racket-mode= with a Podman container I use something like the following in a =.dir-locals.el= file.

Currently this depends on using a branch of =racket-mode= connected to [[https://github.com/greghendershott/racket-mode/pull/748][this open pull request]].

Also that Docker image doesn't really work out of the box, as the REPL can only be loaded if =rackunit= is available.
  ...
  . ((eval
      . (let ((root (project-root (project-current)))
          ...
          (racket-add-back-end
           root
           :racket-program
           (string-join  `("podman" "run" "--rm" "--interactive"
                           ,(concat "--volume="
                                    user-emacs-directory
                                    ":"
                                    user-emacs-directory
