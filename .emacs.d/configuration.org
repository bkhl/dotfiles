...
** Use =avy= to move within visible text
...
in the spirit of Zettelkasten. The custom function =bkhl/zetteldeft-generate-id=
is for generating note IDs. This uses the traditional format with date and time
in minutes, but increases the number to the next free value if necessary. The
format won't be correct on the minute, and may end up generating invalid dates,
but I don't think that's important.

#+begin_src emacs-lisp
(defun bkhl/zetteldeft-generate-id (title &optional filename)
  "Generate Zettel IDs that are always unique."
  (cl-labels
      ((find-first-free (id)
                        (if (zetteldeft--id-available-p id)
                          ...
                          (find-first-free
                           (number-to-string
                            (+ 1 (string-to-number id)))))))
    (find-first-free (format-time-string "%Y%m%d%H%M"))))

(use-package zetteldeft
  :after deft
  :config
  (zetteldeft-set-classic-keybindings)
  (setq zetteldeft-custom-id-function #'bkhl/zetteldeft-generate-zetteldeft
        id-id-regex "[0-9]\\{12\\}"))
#+end_src
