...
**** Typst
***** [[https://codeberg.org/meow_king/typst-ts-mode][typst-ts-mode]] for Typst

#+begin_src emacs-lisp
(use-package typst-ts-mode
  :mode
  ((rx ".typ" eos) . typst-ts-mode)
  :config
  (defun my/config-typst-ts-mode ()
    (setq-local eglot-server-programs
                '((typst-ts-mode . ("tinymist"
                                    :initializationOptions
                                    (:preview (:background (:enabled t)))))))
    (eglot-ensure))
  :hook (typst-ts-mode . my/config-typst-ts-mode))
#+end_src
