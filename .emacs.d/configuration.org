...
To use a language server through [[#eglot][Eglot]] for C, you can do something like this:

+ build a container image containing [[https://github.com/MaskRay/ccls][ccls]]. I have crated a [[https://github.com/lspcontainers/dockerfiles/pull/103][PR to add this to the LSP containers project]].
+ have a ~.dir-locals.el~ file like the below example, which will:
  + set it to run this container for C mode buffers, with the project directory mounted to the same path within the container, to ensure full paths sent to the LSP server matches those on the host system.
  + enable Eglot automatically when opening C mode buffers.
