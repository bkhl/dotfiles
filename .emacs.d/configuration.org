...
Configure =deft=, a plugin to create and search in notes. The call to
=deft-cache-initialize= is there because you can otherwise get errors trying to
create a new note with =zetteldeft= before deft has been used.

#+begin_src emacs-lisp
(use-package deft
  :after org
  :custom
  (deft-directory bkhl/org-directory)
